...
module.exports = {
  async adiciona(usuario) {
    try {
      await dbRun(
        `INSERT INTO usuarios (nome, email, senhaHash, genero, generoPreferencia) 
        VALUES (?, ?, ?, ?, ?)`,
        [usuario.nome, usuario.email, usuario.senhaHash, usuario.genero, usuario.generoPreferencia]
      );
    } catch (erro) {
